...
const BannerHome = () => {
    ...
    useEffect(() => {
        ...
        if(reloadImage) {
            axios(`https://api.themoviedb.org/3/movie/popular?api_key=75f15351c6119a96302b866663e596b0&language=en-US&page=1`)
    ...
    useEffect(() => {
        if (featured) {
            axios(`https://api.themoviedb.org/3/movie/${featured.id}/images?api_key=75f15351c6119a96302b866663e596b0&language=en-US`)
            .then((response) => {
                // derfor den ikke skal return undefined, så vil der komme en blank box
                if (response.data.backdrops[0] !== undefined) {
                    setBackdrop(response.data.backdrop[0])
                } else {
                    setReloadImage(true)
                }
            })
