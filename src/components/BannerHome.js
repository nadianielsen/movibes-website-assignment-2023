...
const BannerHome = () => {
    ...
    return ( 
    <div className="bg-transparent w-[56rem] h-72 rounded-3xl ml-5 mt-10 flex justify-end items-end bg-gradient-to-b from-neutral-700 bg-blend-multiply">
        {/* {error && <p>{error}</p>}
            {loading && <p>Loading...</p>}
            {data?.results?.map(movie => (
                // <img src={`https://image.tmdb.org/t/p/original${movie.poster_path}`} alt="" className="" />
                ))} */}
